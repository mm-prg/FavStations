      ...
      (function attachContextHandlers(btnEl, slotIndex) {
        ...
        function openMenuAt(x, y) {
          ...
          showStationContextMenu(x, y, {
            items: [
              { label: 'Edit station', action: () => openEditorForTemp(slotIndex) },
              { label: 'Delete station', action: () => { if (!confirm('Delete this temp slot?')) return; tempSlots[slotIndex] = null; renderTempSlots(); showToast(`Deleted slot ${slotIndex+1}`); } },
  ...
  function createStationButton(st, idx) {
    ...
    (function attachContextHandlers(btnEl, index) {
      ...
      function openMenuAt(x, y) {
        ...
        showStationContextMenu(x, y, {
          items: [
            { label: 'Edit station', action: () => openEditor(index) },
            { label: 'Delete station', action: async () => { if (!confirm('Delete this station?')) return; stations.splice(index, 1); await persistStations(); renderButtons(); showToast('Deleted'); } },
